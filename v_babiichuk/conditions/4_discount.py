...
# Запитуємо у користувача, чи він є членом клубу (так/ні)
isClubMember = input("Ви є членом клубу покупців? (так/ні): ").lower()  
# Перетворюємо введений рядок до нижнього регістру

# Запитуємо вартість покупки
purchaseAmount = float(input("Введіть вартість покупки в гривнях: "))

# Використовуємо оператор `or` для визначення можливості отримання знижки
