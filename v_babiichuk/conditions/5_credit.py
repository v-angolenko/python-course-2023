...
hasGoodCreditHistory = input("Чи є у вас хороша кредитна історія? (так/ні): ").lower() == "так"
monthlyIncome = int(input("Введіть місячний дохід в гривнях: "))
isLongTermClient = input("Чи є ви постійним клієнтом банку? (так/ні): ").lower() == "так"

# Визначаємо право на кредит за допомогою комбінації операторів `and` та `or`
